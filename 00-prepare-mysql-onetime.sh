...
mysql --defaults-file=$conn_file_user $analysisdb -e "create table if not exists jobinfo(dbid bigint unsigned not null, jobid bigint(20) not null default -1, runtime int(11) not null default 0, enddate date not null default '2000-01-01', cores int(11) not null default 0, mem int(11) not null default 0, gpus int(11) not null default 0, nodes int(11) not null default 0, part varchar(32) not null default '-', account varchar(64) not null default '-', username varchar(64) not null default '-', uid int(10) unsigned not null default 0, primary key(dbid), index(part), index(account), index(username), index(uid));"
...
mysql --defaults-file=$conn_file_user $analysisdb -e "create table if not exists Amazonjobcost(dbid bigint unsigned not null, jobid bigint(20) not null default -1, instancetype varchar(50) not null default '-', origreservedcost int(11) not null default 0, origspotcost int(11) not null default 0, latestreservedcost int(11) not null default 0, latestspotcost int(11) not null default 0, primary key(dbid), index(instancetype));"
